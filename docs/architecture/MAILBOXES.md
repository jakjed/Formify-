...
Aptora uses **two separate mailbox concepts**. They must not be confused in Admin or integrations.
...
| Field | Where | Notes |
| --- | --- | --- |
| Address | `CaptureMailbox.address` | e.g. `{tenant-slug}-invoices@inbound.aptora.local` |
...
**Purpose:** From address when Aptora sends **approval and workflow email** to approvers (assignment, reminder, outcome).
...
| Aptora notifies approver | **Outbound** | Outbound notifications | `TenantOutboundEmail` |
