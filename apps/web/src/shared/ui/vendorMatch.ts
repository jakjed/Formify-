/** Mirror of API vendor-match scoring for client auto-suggest. */
export function scoreVendorName(raw: string, vendorName: string): number {
...
export function bestVendorMatch<T extends { id: string; name: string }>(
  raw: string,
  vendors: T[],
): T | null {
  let best: { row: T; score: number } | null = null;
  for (const v of vendors) {
    const score = scoreVendorName(raw, v.name);
